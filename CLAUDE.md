...
**What it is:** Inspired interpretation of Together AI's design language — near-black hero bands (`#010120`) with a single three-color gradient chrome (logo-blue → sky → periwinkle) alternating with bright white research/pricing/docs bands, knit by a custom display sans + uppercase mono eyebrow. Larch remaps Together's warm orange→magenta to logo-derived blues from `public/images/logo.png`.

**Tokens (use verbatim, no substitutions):**

- **Colors `DESIGN.md:6`** — `primary #000000` / `on-primary #ffffff`, `canvas #ffffff` / `canvas-dark #010120` / `surface-dark-soft #313641`, `ink #000000` / `body #959494` / `hairline #959494`, `accent-blue #0060e0` (logo dominant `0,96,224`) / `accent-sky #3b82f6` / `accent-periwinkle #bdbbff` / `accent-mint #c8f6f9` (+ legacy aliases `accent-orange`→`#0060e0`, `accent-magenta`→`#3b82f6`). Gradient is fixed order `accent-blue → accent-sky → accent-periwinkle` — never cropped to one color, never reordered, never 4th stop (`DESIGN.md:373`).
